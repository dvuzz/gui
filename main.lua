...
local function createOptionHolder(holderTitle, parent, parentTable, subHolder)
	...
	-- Chiều cao tối đa của bảng trước khi hiện thanh cuộn (bạn có thể chỉnh số 400 này)
	local maxHeight = 400 
	...
	-- UPDATE: Sử dụng ScrollingFrame nếu là Menu chính, Frame thường nếu là Folder con
	if subHolder then
		...
		parentTable.content = library:Create("ScrollingFrame", {
			...
			Size = UDim2.new(1, 0, 1, -size),
			BackgroundTransparency = 1,
			ScrollBarThickness = 2, -- Độ dày thanh cuộn
			ScrollBarImageColor3 = library.theme.Accent or Color3.fromRGB(0, 255, 128),
			BorderSizePixel = 0,
	...
	-- UPDATE: Hàm tính toán kích thước mới
	local function updateSize()
		if subHolder then
			-- Logic cũ cho Folder con
			...
			-- Logic MỚI cho Menu chính (có thanh cuộn)
			local contentHeight = layout.AbsoluteContentSize.Y
			parentTable.content.CanvasSize = UDim2.new(0, 0, 0, contentHeight + 10)
			
			-- Tính toán chiều cao cửa sổ, không vượt quá maxHeight
	...
	if not subHolder then
		library:Create("UIPadding", {
			PaddingLeft = UDim.new(0, 5), -- Thêm chút khoảng cách để không bị đè vào thanh cuộn
	...
	closeHolder.InputBegan:connect(function(input)
		if input.UserInputType == ui or input.UserInputType == Enum.UserInputType.Touch then
			...
			if subHolder then
				...
				-- Tween cho Menu chính (có giới hạn chiều cao)
				local contentHeight = layout.AbsoluteContentSize.Y
				local targetHeight = math.min(contentHeight + size + 5, maxHeight)
    ...
    option.main.InputBegan:connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            tweenService:Create(option.mainHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                ImageTransparency = 0, 
                Position = UDim2.new(0, position.X + 240 + 10, 0, position.Y) 
            }):Play()
