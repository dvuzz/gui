...
local httpService = game:GetService("HttpService") -- Cần cho Config System
...
-- Hàm cập nhật vị trí khi kéo (Drag) mượt hơn
local function update(input)
	local delta = input.Position - dragStart
	local yPos = (startPos.Y.Offset + delta.Y) < -36 and -36 or startPos.Y.Offset + delta.Y
    -- Dùng EasingStyle 'Exponential' hoặc 'Back' để mượt hơn
...
-- Tối ưu hóa Chroma (Rainbow) dùng task.spawn và task.wait
local chromaColor = Color3.fromRGB(255, 0, 0)
local rainbowTime = 5

task.spawn(function()
	while task.wait() do -- Dùng task.wait thay vì wait()
		local hue = tick() % rainbowTime / rainbowTime
		chromaColor = Color3.fromHSV(hue, 1, 1)
        -- Nếu muốn tự động cập nhật màu Accent theo Rainbow, bỏ comment dòng dưới:
        -- library.theme.Accent = chromaColor 
	end
end)
...
function library:Init()
	-- Tạo tên ngẫu nhiên để tránh bị game phát hiện (Anti-detection cơ bản)
	local guiName = httpService:GenerateGUID(false)
	
	self.base = self:Create("ScreenGui", {
		Name = guiName,
		ResetOnSpawn = true,
		IgnoreGuiInset = true, -- Giúp UI full màn hình, không bị lệch topbar
		ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	})

	-- Logic tìm Parent xịn nhất
	if gethui then
...
function library:Notify(config)
    ...
    -- Tạo Container một lần duy nhất
    ...
    -- UI Thông báo đẹp hơn (Glass style)
    local notifyFrame = library:Create("Frame", {
        Name = "NotifyFrame",
        Size = UDim2.new(0, 0, 0, 0), -- Bắt đầu nhỏ xíu để animation
        BackgroundColor3 = Color3.fromRGB(25, 25, 25),
        BorderSizePixel = 0,
        Parent = notifyContainer,
        ClipsDescendants = true,
    ...
    local contentFrame = library:Create("Frame", {
        ...
        BackgroundTransparency = 1,
        Parent = notifyFrame,
        Visible = false -- Ẩn nội dung lúc đầu
    })
