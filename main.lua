...
function library:Watermark(info)
    -- Nếu chưa có Base GUI thì không tạo
    if not library.base then return end

    local text = type(info) == "table" and info.text or tostring(info)
    
    -- Nếu watermark đã tồn tại, chỉ cập nhật text
    if library.watermarkElement then
        library.watermarkElement.Label.Text = text
        return library.watermarkElement
    end

    local wm = {}

    -- Tạo khung chứa Watermark (Góc trái trên cùng)
    wm.Frame = library:Create("Frame", {
        Name = "Watermark",
        Position = UDim2.new(0, 15, 0, 15),
        Size = UDim2.new(0, 0, 0, 26), -- Chiều rộng sẽ tự động chỉnh
    ...
    -- Viền màu (Accent)
    wm.Stroke = library:Create("UIStroke", {
        Color = library.theme.Accent,
        Thickness = 1,
        Parent = wm.Frame
    })

    -- Tạo Gradient cho viền (Thêm hiệu ứng đẹp mắt)
    local gradient = library:Create("UIGradient", {
        Color = ColorSequence.new{
    ...
    -- Text hiển thị
    wm.Label = library:Create("TextLabel", {
    ...
    -- Tự động chỉnh kích thước khung theo độ dài chữ
    wm.Label:GetPropertyChangedSignal("TextBounds"):Connect(function()
        tweenService:Create(wm.Frame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, wm.Label.TextBounds.X + 20, 0, 26)
        }):Play()
    end)
    
    -- Cập nhật kích thước lần đầu
    wm.Frame.Size = UDim2.new(0, wm.Label.TextBounds.X + 20, 0, 26)

    -- Hàm cập nhật text từ bên ngoài
    function wm:Set(newText)
        wm.Label.Text = newText
    end

    -- Hàm ẩn hiện
    function wm:Visible(bool)
        wm.Frame.Visible = bool
    end

    -- Lưu vào thư viện để dùng lại
    library.watermarkElement = wm
    return wm
end
