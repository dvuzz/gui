...
function parent:AddParagraph(option)
	...
	function parent:AddPlayerList(option)
		option = typeof(option) == "table" and option or {}
		option.text = tostring(option.text or "Select Player")
		...
		for _, p in pairs(Players:GetPlayers()) do
			table.insert(option.values, p.Name)
		end
		
		
		local listObject = parent:AddList(option)
		
		
		local addedConn = Players.PlayerAdded:Connect(function(p)
			table.insert(listObject.values, p.Name)
			
			listObject:Refresh(listObject.values)
		end)
		
		
		local removedConn = Players.PlayerRemoving:Connect(function(p)
			for i, name in ipairs(listObject.values) do
				if name == p.Name then
					table.remove(listObject.values, i)
					break
				end
			end
			
			
			if listObject.value == p.Name then
				listObject.value = "None" 
				listObject:Set("None")    
			end
