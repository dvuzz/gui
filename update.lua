...
local function createColor(option, parent, holder)
    ...
    -- We create the window specifically for this option to ensure encapsulation
    local pickerFrame = library:Create("ImageButton", {
        Name = "ColorPicker_" .. option.text,
        Size = UDim2.new(0, 260, 0, 270), -- Tăng kích thước để chứa Input Hex/RGB
    ...
    function option:SetColor(col, fromRainbow)
        if typeof(col) == "Color3" then
            ...
            updateVisuals(col, not fromRainbow) -- Don't update inputs constantly if rainbow
    ...
    huePicker.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            isDraggingHue = true
            option:SetRainbow(false) -- Disable rainbow on manual interaction
        end
    end)
    ...
    option.main.MouseButton1Click:Connect(function()
        ...
        if option.open then
            ...
            -- Update UI to current color state
