...
function library:Notify(config)
    ...
    local notifyFrame1 = library:Create("Frame", {
        Name = "NotifyFrame1",
    ...
    local openTween = tweenService:Create(notifyFrame, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0, 0, 0, 0)})
    openTween:Play()
    ...
    task.delay(duration, function()
        if notifyFrame1 then
            local closeTween = tweenService:Create(notifyFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Position = UDim2.new(1, 90, 0, 0)})
            closeTween:Play()
            closeTween.Completed:Connect(function()
                notifyFrame1:Destroy()
            end)
        end
    end)
end
